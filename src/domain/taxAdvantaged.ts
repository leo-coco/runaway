...
/**
 * Apply conversions then RMD to the holdings for one year, mutating `assets`:
 *  1. Each active conversion moves its (inflated) amount from the source account's
...
export const applyForcedFlows = (
  ...
  const { residence, age, birthYear, rmdEnabled, conversions, inflationFactor } = opts;
  ...
  // 2) RMD: force a minimum withdrawal from tax-deferred holdings.
  let rmdGross = 0;
  if (rmdEnabled) {
    ...
    if (fraction > 0) {
      ...
      if (required > 0) {
        const sold = required / deferredBalance;
        for (const h of deferred) {
          if (h.basis !== undefined) h.basis *= 1 - sold;
          h.value -= required * (h.value / deferredBalance);
        }
        rmdGross = required;
      }
    }
